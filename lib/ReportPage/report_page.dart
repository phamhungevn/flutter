...
import 'src/util/extension_string.dart' show StringExtension;

import 'util/examples_descriptor.dart';
...
class ReportPage extends StatefulWidget {
  ...
  @override
  State<ReportPage> createState() => _MyHomePageState();
  static init() {
    var exampleComboToRun = const ReportPage(title: '',).requestedExampleToRun();
  ...
  Tuple2<ExamplesEnum, ExamplesChartTypeEnum> requestedExampleToRun() {
    ...
    const String exampleToRunStr =
    String.fromEnvironment('EXAMPLE_TO_RUN', defaultValue: 'ex10RandomData');
    ExamplesEnum exampleComboToRun = exampleToRunStr.asEnum(ExamplesEnum.values);

    const String chartTypeToShowStr =
    String.fromEnvironment('CHART_TYPE_TO_SHOW', defaultValue: 'lineChart');
    ExamplesChartTypeEnum chartTypeToShow =
    chartTypeToShowStr.asEnum(ExamplesChartTypeEnum.values);
          ...
          children: <Widget>[
            ...
            Expanded(
              ...
              child: Row(
                ...
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Text(exampleSpecific.leftSqueezeText),
                  // '>>>' by default
                  ...
                  Text(exampleSpecific.rightSqueezeText),
                  // '<<' by default
...
class MyLabelCommonOptions extends LabelCommonOptions {
  const MyLabelCommonOptions() : super();
...
/// (by environment we mean the widgets in document_scanner.dart outside the chart).
///
...
class _ExampleDefiner {
  ...
  /// Support for each example manipulate it's environment - the widgets in document_scanner.dart outside the chart.
  ...
  /// - Running:
  ///   ```sh
  ///     flutter run example/lib/document_scanner.dart \
  ...
  Widget createRequestedChart() {
    ...
    List<String> randomDataRowsLegends(int dataRowsCount) {
      ...
      if (dataRowsCount >= 1) {
        defaultLegends.add('Student 1');
      }
      if (dataRowsCount >= 2) {
        defaultLegends.add('Student 2');
      }
      if (dataRowsCount >= 3) {
        defaultLegends.add('Student 3');
      }
      if (dataRowsCount >= 4) {
        defaultLegends.add('Student 4');
      }
      if (dataRowsCount >= 5) {
        defaultLegends.add('Student 5');
      }
      if (dataRowsCount >= 6) {
        defaultLegends.add('Student 6');
      }
      if (dataRowsCount > 6) {
        for (int i = 3; i < dataRowsCount; i++) {
          ...
          defaultLegends.add('OTHER $number');
        }
      }
      return defaultLegends;
    }

    List<double> randomDataOneRow({
    ...
    List<String>? randomDataYLabels(bool useUserProvidedYLabels) {
      ...
      if (useUserProvidedYLabels) {
        yUserLabels =['0%', '100%']; //['10%', '20%', '30%', '40%', '100%'];
      }
      return yUserLabels;
    }

    List<List<double>> randomDataYs(
        int numXLabels, int numDataRows, bool overlapDataYs) {
    ...
    switch (exampleComboToRun) {
      case ExamplesEnum.ex10RandomData:
        ...
        chartData = RandomChartData(
            dataRowsLegends: randomDataRowsLegends(4),
            dataRows: randomDataYs(6, 4, false),
            xUserLabels: randomDataXLabels(6),
            yUserLabels: randomDataYLabels(true),
            chartOptions:
                chartOptions); //generated(chartOptions: chartOptions);
        break;
      ...
      case ExamplesEnum.ex60LabelsIteration1:
        ...
        exampleSideEffects = _ExampleSideEffects()
          ..leftSqueezeText = ''
          ..rightSqueezeText = '';
        break;
